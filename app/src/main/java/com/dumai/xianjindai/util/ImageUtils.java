...
import com.dumai.xianjindai.util.dct.FDCT;
...
public class ImageUtils {
	...
	public static Bitmap getBitmap(String url) {
		...
		try {
			...
			LogUtil.d(ImageUtils.class, "" + e.getMessage());
		} finally {
			try {
				if (is != null) {
					is.close();
	...
	private static boolean checkBitmap(Bitmap bitmap) {
		if (bitmap == null) {
			LogUtil.e(ImageUtils.class, "原图Bitmap为空了");
			return false;
		}

		if (bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
			LogUtil.e(ImageUtils.class, "原图Bitmap大小为0");
			return false;
		}
		return true;
	}
	...
	private static boolean checkSize(int desiredWidth, int desiredHeight) {
		if (desiredWidth <= 0 || desiredHeight <= 0) {
			LogUtil.e(ImageUtils.class, "请求Bitmap的宽高参数必须大于0");
			return false;
		}
		return true;
	}
