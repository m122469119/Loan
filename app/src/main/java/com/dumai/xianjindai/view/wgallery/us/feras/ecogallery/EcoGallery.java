package com.dumai.xianjindai.view.wgallery.us.feras.ecogallery;
...
public class EcoGallery extends EcoGalleryAbsSpinner implements GestureDetector.OnGestureListener {
    ...
    public EcoGallery(Context context, AttributeSet attrs, int defStyle) {
        ...
        int index = a.getInt(R.styleable.WGallery_wGallery_gravity, -1);
        if (index >= 0) {
            setGravity(index);
        }

        int animationDuration = a.getInt(R.styleable.WGallery_wGallery_animationDuration, -1);
        if (animationDuration > 0) {
            setAnimationDuration(animationDuration);
        }

        int spacing = a.getDimensionPixelOffset(R.styleable.WGallery_wGallery_spacing, 0);
        setSpacing(spacing);

        float unselectedAlpha = a.getFloat(R.styleable.WGallery_wGallery_unselectedAlpha, 0.5f);
